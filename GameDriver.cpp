...
void print_successors(OthelloBoard *board) {
	MinimaxPlayer m('O');
	OthelloBoard** s = new OthelloBoard * [16];
	int count = 0;
	int cols[16];
	int rows[16];
	m.get_successors(board, s,cols, rows, count, board->get_p2_symbol());
	std::cout << "#######################################\n";
...
void GameDriver::run() {
	...
	if( board->has_legal_moves_remaining(current->get_symbol())) {
		...
		// Both players can't move, game over    
			std::cout << "minimax utility is: " << m.utility(board) << std::endl;
			std::cout << "Terminal function is: " << m.is_terminal_state(board) << std::endl;
		break;
	    } else {
		cant_move_counter++;
	    }
